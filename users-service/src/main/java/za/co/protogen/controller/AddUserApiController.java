...
public class AddUserApiController implements AddUserApi {

    private final AddUserApiDelegate delegate;

    public AddUserApiController(@Autowired(required = false) AddUserApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new AddUserApiDelegate() {});
    }

    @Override
    public AddUserApiDelegate getDelegate() {
        return delegate;
    }
